...
- [x] T001 Attach `com.apple.developer.private-cloud-compute` to
  waste-sort target entitlements; verify provisioning regenerates without
  error (device build only).
- [x] T002 Create `Core/Intelligence/PCCJudge/` group + add files to both
  targets (app + tests) in the Xcode project.

## Phase 2: Foundational (Blocking Prerequisites)

- [x] T003 [P] Implement `PCCJudgeAvailability.swift` — `#available(iOS 27)`
...
- [x] T006 Implement `PCCRecordStore.swift`: Application Support layout,
  ...
  bookmark.
- [x] T007 Extend settings plumbing four-place style:
...
- [x] T008 [P] [US1] `PCCTriggerPolicyTests.swift`: triggers exactly on the
...
- [x] T009 [P] [US1] `PCCArbiterServiceTests.swift` (mock transport):
...
- [x] T010 [US1] Implement `PCCTriggerPolicy.swift` (pure decision per
  contract rules) — make T008 pass.
- [x] T011 [US1] Implement `PCCArbiterService.swift` behind the protocol:
...
- [x] T012 [US1] Wire trigger evaluation into
...
- [x] T014 [P] [US2] `PCCDatasetExporterTests.swift`: seeded store → range-
...
- [x] T015 [US2] Implement `PCCDatasetExporter.swift`: build bundle dir
...
- [x] T016 [P] [US3] Store tests for prune/range/query edge cases if not
  covered by T006 implementation notes (empty store, corrupt line skipped
  with log, prune spares exported).

### Implementation for User Story 3

- [x] T017 [US3] SettingsView Developer section: judge toggle (bound through
...
- [x] T018 [US3] Foreground maintenance hook: throttled prune call (never on
...
- [x] T019 Run full suite: `xcodebuild test … -destination 'platform=iOS
...
- [x] T020 `swiftlint lint --strict` zero errors; file_length soft caps
  respected (split if warned).
- [x] T021 Update AGENTS.md §7 (Apple-framework features) with one-line PCC
