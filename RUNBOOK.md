...
| App crashes when PCC receives an image | **iOS 27 beta ≤4 + mismatched Xcode beta-1 SDK**: the runtime declared `.vision` but trapped (EXC_BAD_ACCESS) on any image attachment. Fixed by matching toolchain and OS (Xcode 27 beta 6 + matching device beta) | Always keep Xcode beta and device beta on the SAME version. If a future beta reintroduces it, flip the smoke screen's "Attempt image judgments" OFF — the app records honest skips instead of crashing |
| Fine-tuning dataset from an export | Bundle = records.jsonl + crops/ + manifest.json + README | `python3 scripts/prepare_cls_dataset.py <bundle> -o dataset` → ImageFolder train/val (session-aware split, PCC-only labels). Needs ~50+ per class before results mean anything; watch the imbalance warning |
